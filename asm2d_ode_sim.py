...
print("=" * 72)
print("  ASM2d Multi-zone A2O ODE Tabanli Sentetik Veri Ureticisi")
print(f"  Konfigurasyon : 3 bolge (AN+AX+AE), {N_TOTAL_STATES} ODE state")
print(f"  Toplam sim    : {N_TOTAL} saat (isinma={WARMUP_H}h + uretim={SIM_H}h)")
print("=" * 72)
...
k_hyd = 3.0/24.0; K_X = 0.10; eta_NO_hyd = 0.60; eta_fe_hyd = 0.40
K_O2_hyd = 0.20; K_NO3_hyd = 0.50

mu_H = 6.0/24.0; eta_NO_H = 0.80; q_fe = 3.0/24.0
K_F = 4.0; K_A_H = 4.0; b_H = 0.40/24.0; K_fe = 4.0
K_O2_H = 0.20; K_NO3_H = 0.50; K_NH4_H = 0.05; K_P_H = 0.01; K_ALK_H = 0.10

q_PHA = 3.0/24.0; q_PP = 1.5/24.0
K_PP = 0.01; K_MAX = 0.34; K_iPP = 0.02
mu_PAO = 1.0/24.0; eta_NO_PAO = 0.60
K_PHA = 0.01; b_PAO = 0.20/24.0; b_PP = 0.20/24.0; b_PHA = 0.20/24.0
K_A_PAO = 4.0; K_O2_PAO = 0.20; K_NO3_PAO = 0.50; K_NH4_PAO = 0.05
K_PS = 0.20; K_P_PAO = 0.01; K_ALK_PAO = 0.10

mu_AUT = 1.0/24.0; b_AUT = 0.15/24.0
K_O2_AUT = 0.50; K_NH4_AUT = 1.0; K_P_AUT = 0.01; K_ALK_AUT = 0.50

k_PRE = 1.0/24.0; k_RED = 0.60/24.0; K_ALK_PRE = 0.50
...
Q_t = diurnal(t_all, Q_avg, amp_frac=0.40, phase_rad=-np.pi / 6.0, noise_frac=0.15)

KOI_in_t = diurnal(t_all, 350.0, amp_frac=0.50, phase_rad=np.pi / 4.0, noise_frac=0.14)
NH4_in_t = diurnal(t_all, 35.0,  amp_frac=0.45, phase_rad=np.pi / 3.0, noise_frac=0.13)
PO4_in_t = diurnal(t_all, 6.0,   amp_frac=0.30, phase_rad=np.pi / 2.8, noise_frac=0.11)
...
print(f"\n[1/4] Multi-zone A2O ODE simulasyonu baslatiliyor...")
print(f"      Reaktor   : V_AN={V_AN:.0f} + V_AX={V_AX:.0f} + V_AE={V_AE:.0f} = {V_TOTAL:.0f} m3")
print(f"      Akimlar   : Q_avg={Q_avg:.0f} m3/h | Q_IR={Q_IR_factor}*Q | Q_RAS={Q_RAS_factor}*Q")
print(f"      SRT       : {SRT_d:.0f} gun")
print(f"      State     : {N_TOTAL_STATES} ODE state (3 bolge x 18)")
print(f"      Proses    : 21 ASM2d prosesi (her bolge icin)")
print("      BDF yontemiyle cozuluyor (stiff sistem)...")
...
print(f"\n[2/4] '{csv_path}' kaydedildi --> {len(df):,} satir (aerobik bolge efluenti)")

multizone_path = 'synthetic_asm2d_multizone.csv'
...
print(f"      '{multizone_path}' kaydedildi --> 3-bolge state izleme")

print("\n[3/4] Veri Dogrulama Istatistikleri:")
print("-" * 72)
...
korr_DO_NH4 = df['DO'].corr(df['NH4_cikis'])
korr_DO_KOI = df['DO'].corr(df['KOI_cikis'])
korr_DO_PO4 = df['DO'].corr(df['PO4_cikis'])
...
print(f"  DO <-> NH4_cikis kor  : {korr_DO_NH4:.3f}")
print(f"  DO <-> KOI_cikis kor  : {korr_DO_KOI:.3f}")
print(f"  DO <-> PO4_cikis kor  : {korr_DO_PO4:.3f}")

ab_koi = (df['KOI_cikis'] < 125).mean() * 100
ab_nh4 = (df['NH4_cikis'] < 10).mean() * 100
ab_po4 = (df['PO4_cikis'] < 2).mean() * 100
print(f"\n  AB Uyum Oranlari (Egitim Verisi):")
print(f"    KOI < 125 mg/L : {ab_koi:.1f}%")
print(f"    NH4 < 10  mg/L : {ab_nh4:.1f}%")
print(f"    PO4 < 2   mg/L : {ab_po4:.1f}%")

print(f"\n  Bolge bazli ortalama state degerleri:")
print(f"  {'State':<6} {'AN (anaerobik)':>16} {'AX (anoksik)':>16} {'AE (aerobik)':>16}")
print("  " + "-" * 60)
for name in ['SO2', 'SF', 'SA', 'SNO3', 'SPO4', 'XH', 'XPAO', 'XPP', 'XPHA', 'XAUT', 'XTSS']:
    i = S_IDX[name]
    print(f"  {name:<6} {sol_an[:, i].mean():>16.2f} {sol_ax[:, i].mean():>16.2f} {sol_ae[:, i].mean():>16.2f}")

xpao_ae_mean = sol_ae[:, S_IDX['XPAO']].mean()
spo4_an_mean = sol_an[:, S_IDX['SPO4']].mean()
spo4_ae_mean = sol_ae[:, S_IDX['SPO4']].mean()
print(f"\n  EBPR (Biyolojik Fosfor Giderimi) Dogrulama:")
print(f"    XPAO (aerobik) ort = {xpao_ae_mean:.1f} mg/L  [hedef >50]   {'OK' if xpao_ae_mean > 50 else 'YETERSIZ'}")
print(f"    SPO4 (anaerobik)   = {spo4_an_mean:.2f} mg/L [hedef >SPO4_AE]  {'OK' if spo4_an_mean > spo4_ae_mean else 'YETERSIZ'}")
print(f"    SPO4 (aerobik)     = {spo4_ae_mean:.2f} mg/L [P alimi sonrasi]")
print(f"    KOI std (cikis)    = {df['KOI_cikis'].std():.2f} mg/L  [hedef >5]   {'OK' if df['KOI_cikis'].std() > 5 else 'DAR'}")
sa_an_mean = sol_an[:, S_IDX['SA']].mean()
print(f"    SA_AN ort          = {sa_an_mean:.2f} mg/L  [hedef >10]  {'OK' if sa_an_mean > 10 else 'YETERSIZ - XPAO substrat sorunu'}")

print("\n[4/4] Grafikler olusturuluyor...")

fig, axes = plt.subplots(8, 1, figsize=(14, 22), sharex=False)
plot_cols = [
    'KOI_giris', 'NH4_giris', 'PO4_giris', 'DO',
    'KOI_cikis', 'NH4_cikis', 'PO4_cikis', 'SNO3'
]
plot_labels = [
    'COD Influent [mg/L]', 'NH4 Influent [mg/L]', 'PO4 Influent [mg/L]', 'DO Aerobic [mg/L]',
    'COD Effluent [mg/L]', 'NH4 Effluent [mg/L]', 'PO4 Effluent [mg/L]', 'SNO3 Aerobic [mg N/L]'
]
...
fig.suptitle("ASM2d Multi-zone A2O Simulation -- First 14 Days", fontsize=12, fontweight='bold')
plt.tight_layout()
plt.savefig('asm2d_ode_timeseries.png', dpi=150, bbox_inches='tight')
plt.close()

fig_mz, axes_mz = plt.subplots(4, 1, figsize=(14, 12), sharex=True)
mz_cols = ['SPO4', 'XPAO', 'XPP', 'XPHA']
mz_labels = ['SPO4 [mg P/L]', 'XPAO [mg/L]', 'XPP [mg/L]', 'XPHA [mg/L]']
zone_colors = {'AN': '#7C3AED', 'AX': '#D97706', 'AE': '#0EA5E9'}
for ax, col, lbl in zip(axes_mz, mz_cols, mz_labels):
    i = S_IDX[col]
    ax.plot(t_out[:show_h], sol_an[:show_h, i], color=zone_colors['AN'], lw=1.0, alpha=0.9, label='Anaerobic')
    ax.plot(t_out[:show_h], sol_ax[:show_h, i], color=zone_colors['AX'], lw=1.0, alpha=0.9, label='Anoxic')
    ax.plot(t_out[:show_h], sol_ae[:show_h, i], color=zone_colors['AE'], lw=1.0, alpha=0.9, label='Aerobic')
    ax.set_ylabel(lbl, fontsize=9)
    ax.grid(True, alpha=0.28)
    ax.legend(fontsize=8, loc='upper right')
axes_mz[-1].set_xlabel("Time [h]", fontsize=10)
fig_mz.suptitle("EBPR Profile -- Inter-Zone P/PAO Dynamics", fontsize=12, fontweight='bold')
plt.tight_layout()
plt.savefig('asm2d_ode_multizone_ebpr.png', dpi=150, bbox_inches='tight')
plt.close()

fig2, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(17, 5))
sc1 = ax1.scatter(df['DO'], df['NH4_cikis'], c=df['NH4_giris'], cmap='plasma',
                  s=2, alpha=0.35, rasterized=True)
ax1.set_xlabel("DO Aerobic [mg/L]"); ax1.set_ylabel("NH4 Effluent [mg/L]")
ax1.set_title("DO -- NH4_effluent"); ax1.grid(True, alpha=0.3)
plt.colorbar(sc1, ax=ax1, label='NH4 Influent [mg/L]')

sc2 = ax2.scatter(df['DO'], df['KOI_cikis'], c=df['KOI_giris'], cmap='viridis',
                  s=2, alpha=0.35, rasterized=True)
ax2.set_xlabel("DO Aerobic [mg/L]"); ax2.set_ylabel("COD Effluent [mg/L]")
ax2.set_title("DO -- COD_effluent"); ax2.grid(True, alpha=0.3)
plt.colorbar(sc2, ax=ax2, label='COD Influent [mg/L]')

sc3 = ax3.scatter(df['DO'], df['PO4_cikis'], c=df['PO4_giris'], cmap='cividis',
                  s=2, alpha=0.35, rasterized=True)
ax3.set_xlabel("DO Aerobic [mg/L]"); ax3.set_ylabel("PO4 Effluent [mg/L]")
ax3.set_title("DO -- PO4_effluent"); ax3.grid(True, alpha=0.3)
plt.colorbar(sc3, ax=ax3, label='PO4 Influent [mg/L]')

plt.suptitle("ASM2d Multi-zone Correlation Checks", fontsize=12, fontweight='bold')
plt.tight_layout()
plt.savefig('asm2d_ode_physics_check.png', dpi=150, bbox_inches='tight')
plt.close()
...
print(f"  Multi-zone CSV: {multizone_path} (3 bolge state izleme)")
print(f"  State / bolge : {N_STATES} | Toplam: {N_TOTAL_STATES}")
print(f"  Proses        : 21 ASM2d prosesi (her bolge icin)")
print(f"  Grafikler     : asm2d_ode_timeseries.png, asm2d_ode_physics_check.png,")
print(f"                  asm2d_ode_multizone_ebpr.png (YENI)")
print("=" * 72)
